...
from jarvissian.main import main, add_arguments, parse_arguments
from jarvissian.main import (
    handle_sigterm,
    install_signal_handler,
    install_tees,
    flush_tees,
)
from jarvissian.main import activate_logging, get_log_level, print_version
import logging
...
class CalrissianMainTestCase(TestCase):
    ...
    def test_main_calls_cwlmain_returns_exit_code(
        self,
        mock_activate_logging,
        mock_get_log_level,
        mock_flush_tees,
        mock_install_tees,
        mock_memory_parser,
        mock_cpu_parser,
        mock_initialize_reporter,
        mock_write_report,
        mock_install_signal_handler,
        mock_delete_pods,
        mock_add_arguments,
        mock_parse_arguments,
        mock_version,
        mock_runtime_context,
        mock_loading_context,
        mock_executor,
        mock_arg_parser,
        mock_cwlmain,
    ):
        ...
        self.assertEqual(
            mock_add_arguments.call_args, call(mock_arg_parser.return_value)
        )
        self.assertEqual(
            mock_parse_arguments.call_args, call(mock_arg_parser.return_value)
        )
        self.assertEqual(
            mock_get_log_level.call_args, call(mock_parse_arguments.return_value)
        )
        self.assertEqual(
            mock_activate_logging.call_args, call(mock_get_log_level.return_value)
        )
        self.assertEqual(
            mock_memory_parser.parse_to_megabytes.call_args,
            call(mock_parse_arguments.return_value.max_ram),
        )
        self.assertEqual(
            mock_cpu_parser.parse.call_args,
            call(mock_parse_arguments.return_value.max_cores),
        )
        self.assertEqual(
            mock_executor.call_args,
            call(
                mock_memory_parser.parse_to_megabytes.return_value,
                mock_cpu_parser.parse.return_value,
                1,
            ),
        )
        self.assertTrue(mock_runtime_context.called)
        self.assertEqual(
            mock_cwlmain.call_args,
            call(
                args=mock_parse_arguments.return_value,
        ...
        self.assertEqual(
            mock_runtime_context.return_value.select_resources,
        ...
        self.assertEqual(
            mock_initialize_reporter.call_args,
            call(
                mock_memory_parser.parse_to_megabytes.return_value,
                mock_cpu_parser.parse.return_value,
            ),
        ...
        self.assertEqual(
            mock_install_tees.call_args,
            call(
                mock_parse_arguments.return_value.stdout,
                mock_parse_arguments.return_value.stderr,
            ),
        )
    ...
    def test_parse_arguments_exits_without_ram_or_cores(self, mock_sys):
        ...
        mock_parser.parse_args.return_value = Mock(
            max_ram=None, max_cores=None, version=None, conf=None
        )
    ...
    def test_parse_arguments_exits_with_ram_but_no_cores(self, mock_sys):
        mock_parser = Mock()
        mock_parser.parse_args.return_value = Mock(
            max_ram=2048, max_cores=None, version=None, conf=None
        )
        parse_arguments(mock_parser)
        self.assertEqual(mock_sys.exit.call_args, call(1))
    ...
    def test_parse_arguments_succeeds_with_ram_and_cores(self, mock_sys):
        mock_parser = Mock()
        mock_parser.parse_args.return_value = Mock(
            max_ram=2048, max_cores=3, version=None, conf=None
        )
    ...
    def test_install_signal_handler(self, mock_handle_sigterm, mock_signal):
        install_signal_handler()
        self.assertEqual(
            mock_signal.signal.call_args, call(mock_signal.SIGTERM, mock_handle_sigterm)
        )
    ...
    def test_install_tees_handles_stdout(self, mock_sys, mock_os, mock_subprocess):
        ...
        self.assertEqual(
            ...
            call(["tee", "/stdout.txt"], stdin=mock_subprocess.PIPE),
        )
        self.assertEqual(
            mock_os.dup2.call_args,
            call(
                mock_subprocess.Popen.return_value.stdin.fileno.return_value,
                mock_sys.stdout.fileno.return_value,
            ),
        )
    ...
    def test_install_tees_handles_stderr(self, mock_sys, mock_os, mock_subprocess):
        ...
        self.assertEqual(
            mock_subprocess.Popen.call_args,
            call(["tee >&2 /stderr.txt"], stdin=mock_subprocess.PIPE, shell=True),
        )
        self.assertEqual(
            mock_os.dup2.call_args,
            call(
                mock_subprocess.Popen.return_value.stdin.fileno.return_value,
                mock_sys.stderr.fileno.return_value,
            ),
        )
    ...
    def test_install_tees_quotes_if_needed(self, mock_sys, mock_os, mock_subprocess):
        install_tees(
            stdout_path="/stdout file with spaces.txt", stderr_path="/stderr.txt; ls"
        )
        self.assertIn(
            call(["tee", "/stdout file with spaces.txt"], stdin=mock_subprocess.PIPE),
            mock_subprocess.Popen.mock_calls,
        )
        self.assertIn(
            call(["tee >&2 '/stderr.txt; ls'"], shell=True, stdin=mock_subprocess.PIPE),
            mock_subprocess.Popen.mock_calls,
        )
    ...
    @patch("calrissian.main.os")
    @patch("calrissian.main.sys")
    def test_install_tees_does_nothing_without_files(
        self, mock_sys, mock_os, mock_subprocess
    ):
        install_tees()
    ...
    def test_activate_logging(self, mock_logging):
        ...
        self.assertEqual(
            [call(mock_level)] * 6,
            mock_logging.getLogger.return_value.setLevel.mock_calls,
        )
        # addHandler should be called 6 times
        mock_streamhandler = mock_logging.StreamHandler.return_value
        self.assertEqual(
            [call(mock_streamhandler)] * 6,
            mock_logging.getLogger.return_value.addHandler.mock_calls,
        )
