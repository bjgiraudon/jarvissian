...
from jarvissian.executor import IncompleteStatusException
from jarvissian.k8s import (
...
from jarvissian.k8s import CompletionResult, read_file
...
class KubernetesClientTestCase(TestCase):
    ...
    def test_submit_pod(self, mock_podmonitor, mock_get_namespace, mock_client):
        ...
        mock_client.CoreV1Api.return_value.create_namespaced_pod = (
        ...
        self.assertEqual(
            mock_create_namespaced_pod.call_args, call("namespace", mock_body)
        )
    ...
    def test_wait_calls_watch_pod_with_pod_name_field_selector(
        ...
        self.assertEqual(
            ...
            call(
                kc.core_api_instance.list_namespaced_pod,
                kc.namespace,
    ...
    @patch("calrissian.k8s.watch", autospec=True)
    def test_wait_calls_watch_pod_with_imcomplete_status(
        self, mock_watch, mock_get_namespace, mock_client
    ):
    ...
    @patch("calrissian.k8s.watch", autospec=True)
    def test_wait_skips_pod_when_status_is_none(
        self, mock_watch, mock_get_namespace, mock_client
    ):
    ...
    def test_wait_skips_pod_when_state_is_waiting(
        ...
        mock_pod.status.container_statuses[0].state = Mock(
            running=None, waiting=True, terminated=None
        )
        self.setup_mock_watch(mock_watch, [mock_pod])
        kc = KubernetesClient()
        kc._set_pod(Mock())
    ...
    def test_wait_follows_logs_pod_when_state_is_running(
        self, mock_follow_logs, mock_watch, mock_get_namespace, mock_client
    ):
        mock_pod = create_autospec(V1Pod)
        mock_pod.status.container_statuses[0].state = Mock(
            running=True, waiting=None, terminated=None
        )
        self.setup_mock_watch(mock_watch, [mock_pod])
        kc = KubernetesClient()
        kc._set_pod(Mock())
    ...
    def test_wait_raises_exception_when_state_is_unexpected(
        ...
        mock_pod = create_autospec(V1Pod)
        mock_pod.status.container_statuses[0].state = Mock(
            running=None, waiting=None, terminated=None
        )
        self.setup_mock_watch(mock_watch, [mock_pod])
        kc = KubernetesClient()
        kc._set_pod(Mock())
        with self.assertRaisesRegex(
            CalrissianJobException, "Unexpected pod container status"
        ):
            kc.wait_for_completion()
    ...
    def test_raises_on_set_second_pod(self, mock_get_namespace, mock_client):
        ...
        kc._set_pod(Mock())
        with self.assertRaisesRegex(
            CalrissianJobException, "This client is already observing pod"
        ):
            kc._set_pod(Mock())
    ...
    def test_get_pod_for_name_not_found(self, mock_get_namespace, mock_client):
        ...
        kc = KubernetesClient()
        with self.assertRaisesRegex(
            CalrissianJobException, "Unable to find pod with name somepod"
        ):
    ...
    def test_get_pod_for_name_multiple_found(self, mock_get_namespace, mock_client):
        ...
        kc = KubernetesClient()
        with self.assertRaisesRegex(
            CalrissianJobException, "Multiple pods found with name somepod"
        ):
            kc.get_pod_for_name("somepod")

    @patch("calrissian.k8s.os")
    def test_get_current_pod_missing_env_var(
        self, mock_os, mock_get_namespace, mock_client
    ):
        mock_os.environ = {}
        kc = KubernetesClient()
        with self.assertRaisesRegex(
    ...
    @patch("calrissian.k8s.os")
    def test_get_current_pod_with_env_var(
        self, mock_os, mock_get_namespace, mock_client
    ):
        mock_client.CoreV1Api.return_value.list_namespaced_pod.return_value = Mock(
            items=[{"name": "mypod"}]
    ...
    @patch("calrissian.k8s.os")
    def test_should_delete_pod_defaults_yes(
        self, mock_os, mock_get_namespace, mock_client
    ):
    ...
    @patch("calrissian.k8s.os")
    def test_should_delete_pod_reads_env(
        self, mock_os, mock_get_namespace, mock_client
    ):
    ...
    def test_delete_pod_name_calls_api(self, mock_get_namespace, mock_client):
        ...
        self.assertEqual(
            "pod-123",
            mock_client.CoreV1Api.return_value.delete_namespaced_pod.call_args[0][0],
        )

    def test_delete_pod_name_ignores_404(self, mock_get_namespace, mock_client):
        mock_client.CoreV1Api.return_value.delete_namespaced_pod.side_effect = (
    ...
    def test_follow_logs_streams_to_logging(
        self, mock_log, mock_get_namespace, mock_client
    ):
        ...
        self.assertEqual(
            mock_read.call_args,
            call("logging-pod-123", "logging-ns", follow=True, _preload_content=False),
        )
        self.assertEqual(
            mock_log.debug.mock_calls,
            [call("[logging-pod-123] line1"), call("[logging-pod-123] line2")],
        )
        self.assertEqual(
...
class KubernetesClientStatusTestCase(TestCase):
    ...
    def test_multiple_statuses_raises(self):
        ...
        with self.assertRaisesRegex(
            CalrissianJobException, "Expected 0 or 1 containers, found 2"
        ):
