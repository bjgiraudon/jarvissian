...
from jarvissian.executor import Resources, JobResourceQueue, ThreadPoolJobExecutor
from jarvissian.executor import (
...
def make_mock_job(resources):
    return Mock(
        builder=Mock(resources={"ram": resources.ram, "cores": resources.cores})
    )
...
class ThreadPoolJobExecutorTestCase(TestCase):
    ...
    def test_select_resources_raises_if_exceeds(self):
        request = {"ramMin": 2000, "ramMax": 3000, "coresMin": 1, "coresMax": 1}
    ...
    def test_select_resources_returns_fit(self):
        request = {"ramMin": 500, "ramMax": 1500, "coresMin": 1, "coresMax": 1}

        result = self.executor.select_resources(request, Mock())
        self.assertEqual(
            result["ram"], 1000
        )  # When ram max requested exceeds total, result should be total
        self.assertEqual(
            result["cores"], 1
        )  # when cpu max requested is below total, result should be requested
    ...
    def test_restore(self):
        ...
        self.assertEqual(
            self.executor.available_resources, self.executor.total_resources
        )
    ...
    def test_allocate_too_much_raises(self):
        ...
        with self.assertRaisesRegex(
            InconsistentResourcesException, "Available resources are negative"
        ):
            self.executor.allocate(resource, self.logger)
    ...
    def test_restore_over_total_raises(self):
        ...
        with self.assertRaisesRegex(
            InconsistentResourcesException, "Available resources exceeds total"
        ):
    ...
    def test_raise_if_exception_queued_does_nothing_when_no_exceptions(self, mock_wait):
        ...
        self.assertFalse(
            mock_wait.called
        )  # wait should only be called to wait for outstanding futures
    ...
    def test_start_queued_jobs(self, mock_allocate, mock_dequeue):
        ...
        result = self.executor.start_queued_jobs(
            pool_executor, self.logger, mock_runtime_context
        )
        self.assertEqual(
            mock_dequeue.call_args, call(self.executor.available_resources)
        )

        # allocates resources
        self.assertEqual(
            mock_allocate.call_args_list,
            [call(job_resources[0], self.logger), call(job_resources[1], self.logger)],
        )
        # connects builder and output_dirs
        self.assertTrue(
            all([j.builder == mock_runtime_context.builder for j in mock_runnable_jobs])
        )
        self.assertEqual(
            self.executor.output_dirs, {j.outdir for j in mock_runnable_jobs}
        )
    ...
    def test_wait_for_completion(self, mock_first_completed, mock_wait):
        ...
        self.assertEqual(
            mock_wait.call_args, call(mock_futures, return_when=mock_first_completed)
        )
...
class ThreadPoolJobExecutorQueueingTestCase(ThreadPoolJobExecutorTestCase):
    ...
    def test_enqueues_jobs_simple(
        self, mock_start_queued_jobs, mock_raise_if_oversized, mock_enqueue
    ):
        manager = Mock()
        ...
        result = self.executor.enqueue_jobs_from_iterator(
            iterator, self.logger, self.mock_runtime_context, self.mock_pool_executor
        )
        self.assertEqual(expected_calls, manager.mock_calls)
        self.assertEqual(
            result, set()
        )  # Nothing submitted, futures should be an empty set

    @patch("calrissian.executor.JobResourceQueue.enqueue")
    @patch("calrissian.executor.ThreadPoolJobExecutor.raise_if_oversized")
    @patch("calrissian.executor.ThreadPoolJobExecutor.start_queued_jobs")
    @patch("calrissian.executor.ThreadPoolJobExecutor.wait_for_completion")
    def test_enqueues_jobs_with_none(
        self,
        mock_wait_for_completion,
        mock_start_queued_jobs,
        mock_raise_if_oversized,
        mock_enqueue,
    ):
        ...
        expected_calls = [
            ...
            call.start_queued_jobs(
                self.mock_pool_executor, self.logger, self.mock_runtime_context
            ),
        ...
        self.assertEqual(
            mock_enqueue.call_args_list, [call(j) for j in self.jobs_with_none if j]
        )
    ...
    def test_drain_queue(
        self, mock_wait_for_completion, mock_start_queued_jobs, mock_is_empty
    ):
        ...
        self.assertEqual(
            mock_start_queued_jobs.call_args,
        ...
        self.assertEqual(
            mock_wait_for_completion.call_args,
            call({"initial", "submitted"}, self.logger),
        )
    ...
    def test_run_jobs_raises_if_no_lock(self):
        ...
        with self.assertRaisesRegex(
            WorkflowException, "workflow_eval_lock must not be None"
        ):
            self.executor.run_jobs(
                mock_process, mock_job_order, self.logger, mock_runtime_context
            )
    ...
    def test_run_jobs(self, mock_drain_queue, mock_enqueue_jobs, mock_executor):
        ...
        self.executor.run_jobs(
            mock_process, mock_job_order, self.logger, self.mock_runtime_context
        )
        self.assertEqual(
            mock_enqueue_jobs.call_args,
            call(
                mock_job_iterator,
                self.logger,
                self.mock_runtime_context,
                mock_context_executor,
            ),
        )
        self.assertEqual(
            mock_drain_queue.call_args,
            call(
                self.logger,
                self.mock_runtime_context,
                mock_context_executor,
                mock_enqueued_futures,
            ),
        )
