...
from jarvissian.job import INIT_IMAGE_ENV_VARIABLE, DEFAULT_INIT_IMAGE
from cwltool.errors import UnsupportedRequirement
from jarvissian.context import CalrissianRuntimeContext
from jarvissian.k8s import CompletionResult
import threading
...
class ReadYamlTestCase(TestCase):
    ...
    def test_read_yaml(self, mock_yaml, mock_open):
        ...
        self.assertEqual(
            mock_yaml.safe_load.call_args,
            call(mock_open.return_value.__enter__.return_value),
        )
...
class KubernetesPodVolumeInspectorTestCase(TestCase):
    ...
    def test_get_persistent_volumes_dict(self):
        ...
        mock_data1_volume.persistent_volume_claim = Mock(
            claim_name="data1-claim-name", read_only=False
        )
        mock_data2_volume = Mock()
        mock_data2_volume.name = "data2"
        mock_data2_volume.persistent_volume_claim = Mock(
            claim_name="data2-claim-name", read_only=False
        )
    ...
    def test_get_mounted_persistent_volumes(self):
        ...
        kpod.get_persistent_volumes_dict.return_value = {
            "data1": ("data1-claim", False),
            "data2": ("data2-claim", False),
        }
...
class KubernetesVolumeBuilderTestCase(TestCase):
    ...
    def test_add_rw_volume_binding(self):
        ...
        self.assertEqual(
            {
                "name": "claim1",
                "persistentVolumeClaim": {"claimName": "claim1", "readOnly": False},
            },
            self.volume_builder.volumes[0],
        )

        self.assertEqual(0, len(self.volume_builder.volume_mounts))
        self.volume_builder.add_volume_binding(
            "/prefix/1/input1", "/input1-target", True
        )
        self.assertEqual(
            {
                "name": "claim1",
    ...
    def test_add_ro_volume_binding(self):
        ...
        self.assertEqual(0, len(self.volume_builder.volumes))
        self.volume_builder.add_persistent_volume_entry(
            "/prefix/2", None, "claim2", True
        )
        self.assertEqual(
            {
                "name": "claim2",
                "persistentVolumeClaim": {"claimName": "claim2", "readOnly": True},
            },
            self.volume_builder.volumes[0],
        )
        ...
        self.assertEqual(0, len(self.volume_builder.volume_mounts))
        self.volume_builder.add_volume_binding(
            "/prefix/2/input2", "/input2-target", False
        )
        self.assertEqual(
            {
                "name": "claim2",
    ...
    def test_volume_binding_exception_if_not_found(self):
        ...
        with self.assertRaisesRegex(
            VolumeBuilderException, "Could not find a persistent volume"
        ):
            self.volume_builder.add_volume_binding(
                "/prefix/2/input2", "/input2-target", False
            )
    ...
    @patch("calrissian.job.KubernetesPodVolumeInspector")
    def test_add_persistent_volume_entries_from_pod_preserves_readonly(
        self, mock_kubernetes_pod_inspector
    ):
        mock_kubernetes_pod_inspector.return_value.get_mounted_persistent_volumes.return_value = [
...
class KubernetesPodBuilderTestCase(TestCase):
    def setUp(self):
        ...
        self.pod_builder = KubernetesPodBuilder(
            self.name,
    ...
    def test_container_args_with_redirects(self):
        self.assertEqual(
            ["cat > stdout.txt 2> stderr.txt < stdin.txt"],
            self.pod_builder.container_args(),
        )
    ...
    def test_container_resources(self):
        ...
        expected = {"requests": {"cpu": "2", "memory": "256Mi"}}
        self.assertEqual(expected, resources)

    def test_gpu_hints(self):
        self.pod_builder.resources = {"cores": 2, "ram": 256}
        self.pod_builder.requirements = [
            OrderedDict(
                [
        ...
        resources = self.pod_builder.container_resources()
        expected = {
            "requests": {"cpu": "2", "memory": "256Mi", "nvidia.com/gpu": "1"},
        ...
        resources = self.pod_builder.container_resources()
        expected = {
            "requests": {"cpu": "2", "memory": "256Mi", "nvidia.com/gpu": "2"},
    ...
    def test_init_containers_when_stdout_has_path(self):
        ...
        self.assertEqual(
            container["command"], ["/bin/sh", "-c", "mkdir -p out/to; mkdir -p err/to;"]
        )
    ...
    def test_init_container_name_default(self, mock_environ_get):
        ...
        self.assertEqual(
            mock_environ_get.call_args,
            call(INIT_IMAGE_ENV_VARIABLE, DEFAULT_INIT_IMAGE),
        )
    ...
    def test_build(self, mock_random_tag):
        ...
        expected = {
            ...
            "spec": {
                ...
                "nodeSelector": {"disktype": "ssd", "cachelevel": "2"},
                "securityContext": {
                    "runAsUser": os.getuid(),
...
class CalrissianCommandLineJobTestCase(TestCase):
    def setUp(self):
        ...
        self.requirements = [
            {"class": "DockerRequirement", "dockerPull": "dockerimage:1.0"}
        ]
        self.hints = []
        self.name = "test-clj"
    ...
    def test_constructor_calculates_persistent_volume_entries(
        self, mock_volume_builder, mock_client
    ):
        self.make_job()
    ...
    def test_check_requirements_raises_with_docker_build(
        self, mock_volume_builder, mock_client
    ):
        self.requirements = [
            {"class": "DockerRequirement", "dockerBuild": "FROM ubuntu:latest\n"}
        ]
        job = self.make_job()
        with self.assertRaisesRegex(
            UnsupportedRequirement, "DockerRequirement.dockerBuild is not supported"
        ):
            job.check_requirements(self.runtime_context)

    def test_check_requirements_ok_with_empty_requirements(
        self, mock_volume_builder, mock_client
    ):
        self.requirements = []
        job = self.make_job()
    ...
    def test_makes_tmpdir_when_not_exists(
        self, mock_os, mock_volume_builder, mock_client
    ):
    ...
    @patch("calrissian.job.os")
    def test_not_make_tmpdir_when_exists(
        self, mock_os, mock_volume_builder, mock_client
    ):
    ...
    def test_finish_calls_output_callback_with_status(
        self, mock_reporter, mock_volume_builder, mock_client
    ):
        job = self.make_job()
        ...
        job.output_callback.assert_called_with(
            job.collect_outputs.return_value, "success"
        )

    @patch("calrissian.job.Reporter")
    def test_finish_looks_up_codes(
        self, mock_reporter, mock_volume_builder, mock_client
    ):
        job = self.make_job()
        job.successCodes = [
        ...
        job.temporaryFailCodes = [
            2,
        ]
        job.permanentFailCodes = [
            3,
        ]  # also anything not covered
        expected_codes = {
            0: "success",
            1: "success",
            2: "temporaryFail",
        ...
        for code, status in expected_codes.items():
            ...
            job.finish(completion_result, self.runtime_context)
            job.output_callback.assert_called_with(
                job.collect_outputs.return_value, status
            )

    @patch("calrissian.job.Reporter")
    @patch("calrissian.job.os")
    @patch("calrissian.job.shutil")
    def test_finish_removes_stagedir(
        self, mock_shutil, mock_os, mock_reporter, mock_volume_builder, mock_client
    ):
        mock_os.path.exists.return_value = True
        job = self.make_job()
    ...
    @patch("calrissian.job.Reporter")
    @patch("calrissian.job.shutil")
    def test_finish_removes_tmpdir(
        self, mock_shutil, mock_reporter, mock_volume_builder, mock_client
    ):
        job = self.make_job()
        job.tmpdir = "tmpdir"
    ...
    def test__get_container_image_raises_no_default(
        self, mock_volume_builder, mock_client
    ):
        self.requirements = []  # Clear out the dockerimage:1.0 from our requirements
        ...
        with self.assertRaisesRegex(
            CalrissianCommandLineJobException,
            "Please ensure tool has a DockerRequirement with dockerPull",
        ):
    ...
    @patch("calrissian.job.os")
    @patch("calrissian.job.read_yaml")
    def test_create_kubernetes_runtime(
        self,
        mock_read_yaml,
        mock_os,
        mock_pod_builder,
        ...
        mock_volume_builder.return_value.add_emptydir_volume_binding = (
            mock_add_emptydir_volume_binding
        )
    ...
    def test_add_file_or_directory_volume_ro(self, mock_volume_builder, mock_client):
        ...
        self.assertEqual(
            mock_add_volume_binding.call_args, call("/resolved", "/target", False)
        )

    def test_ignores_add_file_or_directory_volume_with_under_colon(
        self, mock_volume_builder, mock_client
    ):
        mock_add_volume_binding = mock_volume_builder.return_value.add_volume_binding
        job = self.make_job()
    ...
    def test_add_writable_file_volume_inplace(self, mock_volume_builder, mock_client):
        ...
        self.assertEqual(
            mock_add_volume_binding.call_args, call("/resolved", "/target", True)
        )

    @patch("calrissian.job.shutil")
    @patch("calrissian.job.ensure_writable")
    def test_add_writable_file_volume_host_outdir_tgt(
        self, mock_ensure_writable, mock_shutil, mock_volume_builder, mock_client
    ):
        ...
        self.assertFalse(mock_add_volume_binding.called)
        self.assertEqual(
            mock_shutil.copy.call_args, call("/resolved", "/host-outdir-tgt")
        )
    ...
    @patch("calrissian.job.shutil")
    @patch("calrissian.job.ensure_writable")
    def test_add_writable_file_volumehost_not_outdir_tgt(
        self,
        mock_ensure_writable,
        mock_shutil,
        mock_tempfile,
        mock_volume_builder,
        mock_client,
        ...
        self.assertEqual(
            mock_shutil.copy.call_args, call("/resolved", "/mkdtemp-dir/resolved")
        )
        # and we expect add_volume_binding called with the tempdir copy
        self.assertEqual(
            mock_add_volume_binding.call_args,
            call("/mkdtemp-dir/resolved", "/target", True),
        )
    ...
    def test_quoted_command_line(
        self, mock_quoted_arg_list, mock_volume_builder, mock_client
    ):
        job = self.make_job()
    ...
    def test_run(self, mock_pod_builder, mock_volume_builder, mock_client):
        ...
        self.assertEqual(
            job.create_kubernetes_runtime.call_args, call(self.runtime_context)
        )
        self.assertEqual(
            job.execute_kubernetes_pod.call_args,
            call(job.create_kubernetes_runtime.return_value),
        )
        self.assertTrue(job.wait_for_kubernetes_pod.called)
        self.assertEqual(
            job.finish.call_args,
            call(job.wait_for_kubernetes_pod.return_value, self.runtime_context),
        )

    @patch("calrissian.job.KubernetesPodBuilder")
    def test_run_uses_tmpdir_lock(
        self, mock_pod_builder, mock_volume_builder, mock_client
    ):
        ...
        expected_calls = [call.enter(), call.make_tmpdir(), call.exit(None, None, None)]
    ...
    @patch("calrissian.job.read_yaml")
    def test_get_pod_nodeselectors(
        self, mock_read_yaml, mock_volume_builder, mock_client
    ):
    ...
    def test_get_from_requirements_raises_not_implemented(
        self, mock_volume_builder, mock_client
    ):
        job = self.make_job()
    ...
    def test_create_runtime_raises_not_implemented(
        self, mock_volume_builder, mock_client
    ):
        job = self.make_job()
    ...
    def test_append_volume(
        self, mock_add_volume_binding, mock_volume_builder, mock_client
    ):
        job = self.make_job()
        job.append_volume([], "source", "target", True)
        self.assertEqual(
            mock_add_volume_binding.call_args, call("source", "target", True)
        )
...
class TotalSizeTestCase(TestCase):
    ...
    def test_counts_in_array(self):
        outputs = {
            "files": [self.make_file(100), self.make_file(200), self.make_file(300)]
        }
