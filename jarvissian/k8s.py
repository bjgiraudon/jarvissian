...
class KubernetesClient(object):
    ...
    @retry_exponential_if_exception_type(
        (
            ApiException,
            HTTPError,
        ),
    ...
    def submit_pod(self, pod_body):
        with PodMonitor() as monitor:
            ...
            log.info(
                "Created k8s pod name {} with id {}".format(
    ...
    def follow_logs(self):
        ...
        for line in self.core_api_instance.read_namespaced_pod_log(
            self.pod.metadata.name, self.namespace, follow=True, _preload_content=False
        ).stream():
    ...
    @retry_exponential_if_exception_type(
        (ApiException, HTTPError, IncompleteStatusException), log
    )
    def wait_for_completion(self) -> CompletionResult:
        ...
        for event in w.stream(
            ...
            log.info(
                "pod name {} with id {} has status {}".format(
                    pod.metadata.name, pod.metadata.uid, status
                )
            )
            if status is None:
                continue
            if self.state_is_waiting(status.state):
                ...
                log.info(
                    "Handling terminated pod name {} with id {}".format(
                        pod.metadata.name, pod.metadata.uid
                    )
                )
    ...
    def _set_pod(self, pod):
        ...
        if self.pod is not None:
            raise CalrissianJobException(
                "This client is already observing pod {}".format(self.pod)
            )
        self.pod = pod
    ...
    def get_first_or_none(
        ...
        if not container_list:  # None or empty list
            ...
            raise CalrissianJobException(
                "Expected 0 or 1 containers, found {}".format(
                    len(container_list), container_list
                )
            )
        else:
    ...
    def _extract_cpu_memory_requests(self, container):
        if container.resources.requests:
            ...
            raise CalrissianJobException(
                "Unable to extract CPU/memory requests, not present"
            )
    ...
    def get_pod_for_name(self, pod_name):
        ...
        pod_list = self.core_api_instance.list_namespaced_pod(
            self.namespace, field_selector=pod_name_field_selector
        )
        if not pod_list.items:
            raise CalrissianJobException(
                "Unable to find pod with name {}".format(pod_name)
            )
        if len(pod_list.items) != 1:
            raise CalrissianJobException(
                "Multiple pods found with name {}".format(pod_name)
            )
...
class PodMonitor(object):
    ...
    def remove(self, pod):
        ...
        if pod.metadata.name in PodMonitor.pod_names:
            ...
            log.warning(
                "PodMonitor {} has already been removed".format(pod.metadata.name)
            )
