...
from jarvissian.k8s import KubernetesClient, CompletionResult
from jarvissian.report import Reporter, TimedResourceReport
...
class KubernetesVolumeBuilder(object):
    ...
    def add_persistent_volume_entries_from_pod(self, pod):
        ...
        for (
            ...
            claim_name,
            read_only,
        ) in inspector.get_mounted_persistent_volumes():
            self.add_persistent_volume_entry(
                mount_path, sub_path, claim_name, read_only
            )
    ...
    def calculate_subpath(source, prefix, parent_sub_path):
        slashed_prefix = os.path.join(
            prefix, ""
        )  # add '/' to end of prefix if it is not there already
    ...
    def add_volume_binding(self, source, target, writable):
        ...
        if not pv:
            raise VolumeBuilderException(
                "Could not find a persistent volume mounted for {}".format(source)
            )
    ...
    def add_emptydir_volume_binding(self, name, target):
        if not name in self.emptydir_volume_names:
            ...
            raise VolumeBuilderException(
                "Could not find an emptyDir volume named {}".format(name)
            )
        volume_mount = {"name": name, "mountPath": target}
        self.volume_mounts.append(volume_mount)
            ...
            containers.append(
                {
                    ...
                    "image": os.environ.get(
                        INIT_IMAGE_ENV_VARIABLE, DEFAULT_INIT_IMAGE
                    ),
    ...
    def container_resources(self):
        ...
        for requirement in self.requirements:
            if requirement["class"] in [
                ...
                resource_bound = "requests"
                container_resources[resource_bound]["nvidia.com/gpu"] = str(
                    requirement["cudaDeviceCountMin"]
                )
                if "limits" in container_resources:
                    resource_bound = "limits"
                    container_resources[resource_bound]["nvidia.com/gpu"] = str(
                        requirement["cudaDeviceCountMax"]
                    )
                else:
                    container_resources["limits"] = {
                        "nvidia.com/gpu": str(requirement["cudaDeviceCountMax"])
                    }
...
class CalrissianCommandLineJob(ContainerCommandLineJob):
    ...
    def dump_tool_logs(
        self, name, completion_result: CompletionResult, runtime_context
    ):
        """
        Dumps the tool logs
        """
        ...
        with open(log_filename, "w") as f:
            for log_entry in completion_result.tool_log:
                f.write(
                    f"{log_entry['timestamp']} - {log_entry['pod']} - {log_entry['entry']}\n"
                )
    ...
    def get_security_context(self, runtimeContext):
        if not runtimeContext.no_match_user:
            return {"runAsUser": os.getuid(), "runAsGroup": os.getgid()}
        else:
            return {}
    ...
    def create_kubernetes_runtime(self, runtimeContext):
        ...
        self._add_volume_binding(
            os.path.realpath(self.outdir), self.builder.outdir, writable=True
        )
        ...
        if self.generatemapper is not None:
            ...
            any_path_okay = (
                self.builder.get_requirement("DockerRequirement")[1] or False
            )
            self.add_volumes(
                self.generatemapper,
                runtime,
    ...
    def add_file_or_directory_volume(
        ...
        if not volume.resolved.startswith("_:"):
            self._add_volume_binding(
                volume.resolved, volume.target
            )  # this one defaults to read_only
    ...
    def add_writable_file_volume(
        ...
        if self.inplace_update:
            ...
            if host_outdir_tgt:
                ...
                log.debug(
                    "shutil.copy({}, {})".format(volume.resolved, host_outdir_tgt)
                )
        ...
        if volume.resolved.startswith("_:"):
            ...
            if not host_outdir_tgt:
                ...
                new_dir = os.path.join(
                    tempfile.mkdtemp(dir=self.tmpdir), os.path.basename(volume.target)
                )
            ...
            if self.inplace_update:
                ...
                if not host_outdir_tgt:
                    ...
                    log.debug(
                        "shutil.copytree({}, {})".format(volume.resolved, new_dir)
                    )
                    shutil.copytree(volume.resolved, new_dir)
                    self._add_volume_binding(new_dir, volume.target, writable=True)
                else:
                    log.debug(
                        "shutil.copytree({}, {})".format(
    ...
    def run(self, runtimeContext, tmpdir_lock=None):
        ...
        if completion_result.exit_code != 0:
            log_main.error(
                f"ERROR the command below failed in pod {get_pod_name(pod)}:"
            )
    ...
    def setup_kubernetes(self, runtime_context):
        cuda_req, _ = self.get_requirement(
            "http://commonwl.org/cwltool#CUDARequirement"
        )
        ...
        if cuda_req:
            if runtime_context.max_gpus:
                ...
                self.builder.resources["cudaDeviceCount"] = max(
                    cuda_req["cudaDeviceCountMin"], cuda_req["cudaDeviceCountMax"]
                )
                self.builder.resources["cudaDeviceCountMin"] = cuda_req[
                    "cudaDeviceCountMin"
                ]
                self.builder.resources["cudaDeviceCountMax"] = cuda_req[
                    "cudaDeviceCountMax"
                ]
            else:
                raise WorkflowException(
                    "Error: set --max-gpus to run CWL files with the CUDARequirement"
                )
