...
from jarvissian.executor import ThreadPoolJobExecutor
from jarvissian.context import CalrissianLoadingContext, CalrissianRuntimeContext
from jarvissian.version import version
from jarvissian.k8s import delete_pods
from jarvissian.report import initialize_reporter, write_report, CPUParser, MemoryParser
...
def activate_logging(level):
    ...
    for logger in loggers:
        ...
        logging.getLogger("calrissian.{}".format(logger)).addHandler(
...
def add_arguments(parser):
    parser.add_argument(
        "--max-ram",
        type=str,
        help="Maximum amount of RAM to use, e.g 1048576, 512Mi or 2G. Follows k8s resource conventions",
    )
    parser.add_argument(
        "--max-cores", type=str, help="Maximum number of CPU cores to use"
    )
    parser.add_argument(
        "--max-gpus", type=str, nargs="?", help="Maximum number of GPU cores to use"
    )
    parser.add_argument(
        "--pod-labels",
        type=Text,
    ...
    parser.add_argument(
        ...
        type=Text,
        nargs="?",
        help="YAML file of environment variables to add at runtime to Pods submitted",
    )
    parser.add_argument(
        "--pod-nodeselectors",
        type=Text,
        nargs="?",
        help="YAML file of node selectors to add to Pods submitted",
    )
    parser.add_argument(
        "--pod-serviceaccount",
        type=str,
    ...
    parser.add_argument(
        ...
        type=Text,
        nargs="?",
        help="Base path for saving the tool logs",
    )
    parser.add_argument(
        "--conf",
        help="Defines the default values for the CLI arguments",
        action="append",
    )
...
def parse_arguments(parser):
    ...
    with contextlib.suppress(KeyError, FileNotFoundError):
        with open(
            os.path.join(os.environ["HOME"], ".calrissian", "default.json"), "r"
        ) as f:
...
def install_tees(stdout_path=None, stderr_path=None):
    ...
    if stdout_path:
        stdout_tee_process = subprocess.Popen(
            ["tee", stdout_path], stdin=subprocess.PIPE
        )
    ...
    if stderr_path:
        ...
        stderr_tee_process = subprocess.Popen(
            ["tee >&2 {}".format(shlex.quote(stderr_path))],
